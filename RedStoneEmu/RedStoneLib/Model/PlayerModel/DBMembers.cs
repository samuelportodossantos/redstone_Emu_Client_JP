...
namespace RedStoneLib.Model
{
    ...
    public partial class Player
    {
        ...
        {
            get => string.Join(",", EquipmentItems?.Select(t => t.UniqueID).ToArray() ?? new int[] {
        ...
        {
            get => string.Join(",", BeltItems?.Select(t => t.UniqueID).ToArray() ?? new int[] { 0, 0, 0, 0, 0 });
        ...
        {
            get => string.Join(",", InventoryItems?.Select(t => t.UniqueID).ToArray() ?? new int[] {
        ...
        private string Skills_DB
        {
            ...
            set
            {
                ...
                {
                    var splited = t.Split(':');
                    return new PlayerSkill { Index = ushort.Parse(splited[0]), Level = ushort.Parse(splited[1]) };
                }).ToArray();
            }
        }
        ...
        private string ProgressQuests_DB
        {
            ...
            set
            {
                if (value != "")
                {
                    var quests = value.Split(',').Select(t =>
                    {
                        var splited = t.Split(':');
                        return new ProgressQuest(int.Parse(splited[0]), int.Parse(splited[1]));
                    }).ToArray();
                    ProgressQuests = Enumerable.Range(0, quests.Length).ToDictionary(t => t, t => quests[t]);
                }
            }
        }
        ...
        private string Titles_DB
        {
            ...
            set
            {
                if (value != "")
                {
                    Titles = value.Split(',').Select(t =>
                    {
                        var splited = t.Split(':');
                        return new { Index = byte.Parse(splited[0]), Level = byte.Parse(splited[1]) };
                    }).ToDictionary(t => t.Index, t => t.Level);
                }
            }
        }
    }
