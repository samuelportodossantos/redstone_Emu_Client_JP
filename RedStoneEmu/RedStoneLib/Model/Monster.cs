...
namespace RedStoneLib.Model
{
    ...
    public class Monster : Base.Actor
    {
        ...
        public byte CursorType { get; private set; } = 1;
        ...
        /// 原点
        /// </summary>
        private readonly Point<uint> OriginalPos;

        /// <summary>
        /// 移動速度
        /// </summary>
        public override ushort MoveSpeed { get => ABase.MoveSpeed; set => throw new NotImplementedException(); }
        ...
        public Monster(MapActorSingle single, MapActorGroup group, ushort charID, ushort mapSerial)
            : base(single, group, charID, mapSerial)
        {
            Image = group.ImageID;
            ...
            //原点
            OriginalPos = new Point<uint> { X = single.Point.X, Y = single.Point.Y };

            //クエスト無効
            IsQuestParticipants = false;
        }
        ...
        public void SetRandomPos(Map map, int range = 4)
        {
            //範囲
            int minX = (int)(OriginalPos.X - 64 * range / 2);
            int maxX = (int)(OriginalPos.X + 64 * range / 2);
            int minY = (int)(OriginalPos.Y - 32 * range / 2);
            int maxY = (int)(OriginalPos.Y + 32 * range / 2);
            //見切れ
            minX = Math.Max(0, minX);
            maxX = Math.Min((int)map.Size.Width*64, maxX);
            minY = Math.Max(0, minY);
            maxY = Math.Min((int)map.Size.Height*32, maxY);
            ...
            do
            {
                x = Helper.StaticRandom.Next(minX, maxX);
                y = Helper.StaticRandom.Next(minY, maxY);
            } while (map.GetBlock(x/64, y/32) != 0);
            PosX = (ushort)x;
            PosY = (ushort)y;
        }
    }
