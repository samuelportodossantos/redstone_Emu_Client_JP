...
namespace RedStoneEmu.Packets.Handlers.GameLogin
{
    ...
    class UpdateReady : PacketHandler
    {
        public override void HandlePacket(Client context, PacketReader reader, uint size)
        {
            ...
            context.SendPacket(new MinimumActorInfoList(context.GetMapActors(context.User.CharID).Where(actor =>
            {
                /*if (actor is Monster monster) return monster.Spawned;
                else if (!isNotMe(actor)) return false;
                else return true;*/
                return isNotMe(actor);
            }).ToList()));

            //npc verysimple リスポーンしてないモンスターは出さない
            context.SendPacket(new VerySimpleActorInfoListPacket(context.GetMapActors().Where(actor =>
            {
                if (actor is Monster monster) return monster.Spawned;
                else return isNotMe(actor);
            }).ToList()));

            //露店情報
            var shops = MAPServer.AllMapServers[context.User.MapSerial].PitchmanShops;
            if (shops.Count > 0)
            {
                using (PacketWriter writer = new PacketWriter())
                {
                    writer.Write((ushort)shops.Count);
                    foreach (Pitchman shop in shops)
                    {
                        writer.Write(shop.OutwardData());
                    }
                    context.SendPacket(new SimplePitchmanShopInfoListPacket(writer.ToArray()));
                }
            }
